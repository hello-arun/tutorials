...
figsize = [70, 70*3.65/2.58]  # in mm
subplots = [1, 1]
fig_name = "stress-strain.png"
# [[[xlabel,ylabel]]]
labels = [[["Strain", "Stress(GPa)"]]]
limits = [[[0, 1.6, 0, 110]]]  # axes limits [[[xmin,xmax,ymin,ymax]]]

# tick location [[[x_major,x_minor,y_major,y_minor]]]
major_minors = [[[0.2, 0.2, 10, 10]]]


def plot(axes):
    import pandas as pd
    GPa = 0.000101325   # 1KCal/mol=0.043 eV
    ax1 = axes[0, 0]
    configs=["111111101111111","111111000111111","001001000100100"]
    for config in configs:
        data = pd.read_csv(f"./{config}/results/log-stress-strain.csv",comment="#",delimiter=",",skipinitialspace=True)
        strain=data["strain"]
        real_vol=data["lx-d"]*data["ly-d"]*data["thickness"] # volume of only dynamic group
        sigma_xx=data["sigma_xx"]*GPa/real_vol
        # sigma_yy=GPa*data["sigma_yy"]/real_vol
        # ax1.plot(data["time"],(data["pe"]+data["ke"])/data["atoms"], label=f"TotEn")
        ax1.plot(strain,sigma_xx, label=f"{config}")
