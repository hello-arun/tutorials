...
def plot(axes):
    ...
    ax1 = axes[0, 0]
    data = pd.read_csv("./results/log-stress-strain.csv",comment="#",delimiter=",",skipinitialspace=True)
    strain=data["strain"]
    real_vol=data["lx-d"]*data["ly-d"]*data["thickness"] # volume of only dynamic group
    sigma_xx=GPa*data["sigma_xx"]/real_vol
    sigma_yy=GPa*data["sigma_yy"]/real_vol
    # ax1.plot(data["time"],(data["pe"]+data["ke"])/data["atoms"], label=f"TotEn")
    ax1.plot(strain,sigma_xx, label=f"Sigma_xx")
    ax1.plot(strain,sigma_yy, label=f"Sigma_yy")
