...
mm=1/25.4
plt.style.use('/ibex/scratch/jangira/qe/npr/ZScripts/modules/allCombined/bands/plot_style.txt')
fig, ax = plt.subplots(1,1)
# fig, ( ax1) = plt.subplots(1,1)
fig.set_size_inches(89*mm,75*mm)

# ax1.set_xlabel(f'Electric field (V/nm)')
# ax1.set_ylabel(f'Poisson\'s ratio')
...
for i in bands: #Here we plots the bands
    ax.plot(i[:,0],i[:,1]-fermi_shift,color=args.color,linestyle=args.ls,lw=args.lw)
...
plt.grid()
plt.tight_layout()
plt.title(f'Bands {float(args.title)*514.22:0.0f} V/nm')
