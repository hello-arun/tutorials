...
cd ../
SRC_DIR=$PWD
rm -f $PREFIX.etot_vs_rho
for rho in $Rhos
do
awk -v dgs="$dgs" -v wfc="$wfc" -v rho="$rho" -v k="$k" -v vk="-1" '/ecutwfc/ { $3 = wfc } 
/K_POINTS/ {vk = NR} NR == vk+1 {$0 = k" "k" "1" 0 0 0"} 
/degauss/ {$3= dgs}
/ecutrho/ {$3 = rho} {print $0} ' $PREFIX.scf.in > $PREFIX.scf.${rho}rho.in
mpirun -np 24 pw.x <$PREFIX.scf.${rho}rho.in> $PREFIX.scf.${rho}rho.out
awk -v x=$rho '/^!/ {print x, $5}' $PREFIX.scf.${rho}rho.out >> $PREFIX.etot_vs_rho
done