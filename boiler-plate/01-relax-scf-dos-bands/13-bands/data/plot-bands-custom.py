...
x_ticks=np.loadtxt(fname="KLABELS",skiprows=1,comments="*",usecols=[1],dtype=float)
x_tick_labels=["$\Gamma$","X","S","Y","$\Gamma$"]
assert len(x_tick_labels)==len(x_ticks)
print(x_tick_labels,x_ticks)
limits = [
    [[x_ticks[0], x_ticks[-1], -4, 4]],
...
def plot(axes,fig):
    ...
    # set_legend(axes)
    ax1.set_ylim(-4,4)
    ax1.text(0.13,0.4,f"{CBM-VBM:0.2f} eV")
    ax1.set_title(r"--")
    # Plot the arrow
