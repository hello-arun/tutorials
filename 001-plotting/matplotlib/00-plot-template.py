...
def init(figsize, subplts):
    ...
    plt.rc("xtick", labelsize=12, direction="in", top=True, bottom=True)
    plt.rc("ytick", labelsize=12, direction="in", right=True, left=True)
    plt.rc(
        "legend",
        fontsize=12,
        title_fontsize=12,
...
subplots = [2, 2]
figsize = [150, 150]  # in mm
labels = [
    [[r"$\epsilon_{11}$", r"$\mathrm{\epsilon_{11}}$"], [
        r"$\sin(\theta_{x\mathrm{y}})$", "y12"]],
    [["Stress", "Strain"], [r"$\mathrm{\sin(\theta_{xy})}$", "y22"]],
]  # [[[xlabel,ylabel]]]
limits = [
    [[0, 9, -2, 12], [-1, 11, -1, 10]],
    [[-1, 11, -1, 11], [-1, 11, -1, 11]],
...
def plot(axes):
    ...
    axes[0][0].text(0.2,9,r"$\epsilon_{xy}$ \epsilon_{xy}")
    axes[0][0].text(0.2,8,r"$\mathrm{\epsilon_{xy}}$ \mathrm{\epsilon_{xy}}")
    axes[0][0].text(0.2,7,r"$\varepsilon_{xy}$ \varepsilon_{xy}")
    axes[0][0].text(0.2,6,r"$\mathcal{E}_{xy}$ \mathcal{E}_{xy}")
...
fig.subplots_adjust(
    ...
    wspace=0.2)
fig.tight_layout()
fig.savefig("00-fig-template.svg")
plt.show()
0