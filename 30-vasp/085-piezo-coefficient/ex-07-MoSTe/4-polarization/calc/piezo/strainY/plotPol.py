...
polFileName=sys.argv[1]
print(polFileName)
suffix=polFileName[-5]
...
major_minors = [[[1.0, 0.2, 20, 10]]]  # tick location [[[x_major,x_minor,y_major,y_minor]]]

mm=1/25.40
fig, axes = plt.subplots(nrows=subplots[0], ncols=subplots[1], squeeze=False)
[[ax]] = axes
fig.set_size_inches(figsize[0] * mm, figsize[1] * mm)
dfPol = pd.read_csv(polFileName,skipinitialspace=True, comment="#")
dfPol=dfPol.dropna()
print(dfPol)
strainAxisLabel=os.getcwd().split("/")[-1][-1]
dfPol['strainAxis'] = {'X':1,'Y':2,'Z':3}[strainAxisLabel]
dfPol['polQuantaAxisLabel'] = dfPol['polAxis'].map({1:'a',2:'b',3:'c'})
print(dfPol)
polQuantaAxisLabel, polAxis, strainAxis = dfPol.iloc[0][['polQuantaAxisLabel', 'polAxis', 'strainAxis']]
print("Debug", strainAxisLabel, polQuantaAxisLabel, polAxis, strainAxis)
strain = dfPol["epsilon"]
height = dfPol["c"] # Height in Angstrom
refIndex = strain.index[strain == 0][0]
pTot = dfPol["pIon"]+dfPol["pEle"]
quanta = dfPol[polQuantaAxisLabel]/dfPol["vol"]
...
message="e_{"+f"{polAxis}{strainAxis}"+"}"+f"$\n${(correctionFactor+np.polyval(slope_coef,0))/100:.2f} \\times 10^{{-10}}"+r"\ \mathrm{C/m}"
print(f"# {message}")
ax.text(0.5,0.8,f"${message}$",ha='center',va='center',transform=ax.transAxes)
...
fig.savefig(f"fig-pol{suffix}VsStrain{strainAxisLabel}.svg")
plt.show()
