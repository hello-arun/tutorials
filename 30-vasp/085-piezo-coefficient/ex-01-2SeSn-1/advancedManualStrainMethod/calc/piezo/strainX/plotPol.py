...
dfPol['polQuantaAxisLabel'] = dfPol['polAxis'].map({1:'a',2:'b'})
print(dfPol)
...
message="e_{"+f"{polAxis}{strainAxis}"+"}"+f"={(correctionFactor+np.polyval(slope_coef,0))/100:.0f} \\times 10^{{-10}}"+r"\ \mathrm{C/m}"
print(f"# {message}")
ax.text(0,40,f"${message}$",ha='center')
