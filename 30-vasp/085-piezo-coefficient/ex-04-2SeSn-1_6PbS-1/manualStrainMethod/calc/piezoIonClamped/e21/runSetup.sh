...
for strain in $strains ; do 
    ...
    cd $srcDIR
    cp KPOINTS POSCAR INCAR POTCAR runCalcPolarization-*.sbatch $calcDIR/

    cd $calcDIR
    strain_along $axis $strain POSCAR POSCAR

    # For Shaheen
    sed -i "s/__jobName/${axis^^}${strain}/" runCalcPolarization-shaheen.sbatch
    sbatch runCalcPolarization-shaheen.sbatch
done
